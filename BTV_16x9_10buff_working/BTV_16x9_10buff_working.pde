...
void setup(){
  ...
  printArray(cameras);
  h = height/9;
  cam = new Capture(this,320,240,cameras[0],30); //use 30 or 3 960X540 when c920 is connected
  cam.start();
...
void draw() {
  if(cam.available()) {
    ...
    buffer7[iWrite7] = cam.get(160,50,h,h);
    buffer8[iWrite8] = cam.get(160,50,h,h);
    buffer9[iWrite9] = cam.get(160,50,h,h);
    buffer10[iWrite10] = cam.get(160,50,h,h);
